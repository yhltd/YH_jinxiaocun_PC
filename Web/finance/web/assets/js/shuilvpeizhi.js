...
function getList() {
    var shuilv = $("#shuilv").val();  // 修改：获取税率值
    var linjiezhi = $("#linjiezhi").val();  // 修改：获取临界值
...
function setTable(data) {
    $('#data-table').datagrid({
        ...
        columns: [[
            ...
            { field: 'shuilv', align: 'center', title: '税率', width: 120 },
            { field: 'linjiezhi', align: 'center', title: '临界值', width: 200 },
        ]]
    })
...
function toUpd() {
    ...
    if (checkForm(params)) {
        ...
        var updateData = {
            id: item.id,
            shuilv: params.shuilv,
            linjiezhi: params.linjiezhi
        };

        $.ajax({
            type: 'Post',
            url: "web_service/shuilvpeihzi.asmx/upd",
            data: {
...
function toAdd() {
    ...
    if (checkForm(params)) {
        // 修改：直接使用params，不需要重新构建
        $.ajax({
            type: 'Post',
            url: "web_service/shuilvpeihzi.asmx/add",
            data: {
...
function checkForm(params) {
    if (!params.shuilv || params.shuilv == "") {
        alert("请输入税率")
        return false;
    } else if (!params.linjiezhi || params.linjiezhi == "") {
        alert("请输入临界值")
        return false;
    }
...
function del(rows) {
    ...
    $.ajax({
        type: 'Post',
        url: "web_service/shuilvpeihzi.asmx/del",
        data: {
...
function toExcel() {
    var shuilv = $("#shuilv").val();  // 修改：获取税率
    var linjiezhi = $("#linjiezhi").val();  // 修改：获取临界值

    $.ajax({
        type: 'Post',
        url: "web_service/shuilvpeihzi.asmx/getAllList",
        beforeSend: function () {
            $.messager.progress({
                title: '提示',
                msg: '正在加载',
        ...
        success: function (data) {
            ...
            if (result.code == 200) {
                ...
                for (var i = 0; i < array.length; i++) {
                    var body = {
                        shuilv: array[i].shuilv,  // 修改：字段名
                        linjiezhi: array[i].linjiezhi  // 修改：字段名
                    }
                    header.push(body)
                }
                title = ['税率', '临界值']  // 修改：表头
                JSONToExcelConvertor(header, "税率配置报表", title)
            }
        },
        error: function (err) {
            alert("错误！")

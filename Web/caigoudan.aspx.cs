...
namespace Web
{
    public partial class caigoudan : System.Web.UI.Page
    {
        ...
        public string insert_ruku(string list)
        {
            try
            {
                ...
                if (infoList != null)
                {
                    ...
                    if (data.ContainsKey("rukuStatus"))
                        rukuStatus = data["rukuStatus"].ToString();
                }

                int result = 0; // 声明在外部，确保两个分支都能访问

                if (rukuStatus == "是")
                {
                    TuiHuomxModel TuiHuomxModel = new TuiHuomxModel();
                    int result1 = TuiHuomxModel.add(infoList, user.gongsi, user.name, "采购", rwarehouse, "已入库");

                    MingxiModel mingXiModel = new MingxiModel();
                    result = mingXiModel.add(infoList, user.gongsi, user.name, "入库", rwarehouse);

                }
                else
                {
                    TuiHuomxModel TuiHuomxModel = new TuiHuomxModel();
                    result = TuiHuomxModel.add(infoList, user.gongsi, user.name, "采购", rwarehouse, "");
                }

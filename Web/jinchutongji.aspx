<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="jinchutongji.aspx.cs" Inherits="Web.jinchutongji" %>
    ...
    <div class="container">
            ...
            <div class="row">
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        <label for="mxtype"><i class="fas fa-exchange-alt mr-2"></i>进出类型</label>
                        <select id="mxtype" class="form-control">
        ...
        <div class="tab-content" id="chartTabsContent">
            ...
            <div class="tab-pane fade show active" id="lineChart" role="tabpanel">
                <h4 class="chart-title">进出明细趋势分析</h4>
            ...
            <div class="tab-pane fade" id="barChart" role="tabpanel">
                <h4 class="chart-title">进出明细排行对比</h4>
                <div class="chart-container">
            ...
            <div class="tab-pane fade" id="pieChart" role="tabpanel">
                <h4 class="chart-title">进出明细分布比例</h4>
                <div class="chart-container">
            ...
            <div class="tab-pane fade" id="dataTable" role="tabpanel">
                <h4 class="chart-title">进出明细详细数据</h4>
...
<%--            <button id="exportExcel" class="btn btn-success mr-2">
    ...
    <script>
        ...
        // 修改loadReturnData函数中的AJAX调用
        function loadReturnData() {
            showLoading(true);
    
            var startDate = $("#startDate").val();
            var endDate = $("#endDate").val();
            var mxtype = $("#mxtype").val();  // 添加mxtype参数
    
            $.ajax({
                url: 'jinchutongji.aspx/GetAllSalesReturns',  // 保持WebMethod名称不变
                type: 'POST',
                contentType: 'application/json',
                data: JSON.stringify({
                    mxtype: mxtype,  // 添加mxtype参数
                    startDate: startDate,
                    endDate: endDate
                }),
        ...
        // 新增：更新进出类型选项函数
        function updateMxtypeOptions() {
            var mxtypeSelect = $("#mxtype");
            var currentMxtype = mxtypeSelect.val();
    
            // 不需要重新生成选项，因为选项是固定的
            // 恢复之前的选择
            if (currentMxtype) {
                mxtypeSelect.val(currentMxtype);
            }
        }
        ...
        function applyFrontendFilters() {
            ...
            // 应用进出类型筛选
            var mxtype = $("#mxtype").val();
            if (mxtype) {
                var tempData = [];
                for (var i = 0; i < filteredData.length; i++) {
                    if (filteredData[i].Unit === mxtype) {  // Unit字段对应mxtype
        ...
        function updateStatCards(totalQuantity, totalAmount, productCount) {
            ...
            cardsHtml += '        <div class="stat-label">总数量</div>';
            cardsHtml += '    </div>';
            cardsHtml += '</div>';
            ...
            cardsHtml += '        <div class="stat-label">总金额</div>';
            cardsHtml += '    </div>';
            cardsHtml += '</div>';
            ...
            cardsHtml += '        <div class="stat-label">平均数量</div>';
            cardsHtml += '    </div>';
            cardsHtml += '</div>';
        ...
        function getChartDataByType(chartType, dateMap, productMap, categoryMap, warehouseMap) {
            ...
            switch(chartType) {
                ...
                case 'mxtype':
                    ...
                    for (var i = 0; i < filteredData.length; i++) {
                        var item = filteredData[i];
                        var mxtype = item.Unit || "未知类型";
                        if (mxtypeMap[mxtype]) {
                            mxtypeMap[mxtype] += 1; // 或者使用数量：item.RawQuantity
                        } else {
                            mxtypeMap[mxtype] = 1;
                        }
                    }
                    ...
                    // 柱状图：进出类型排行
                    var mxtypeEntries = [];
                    for (var mxtype in mxtypeMap) {
                        if (mxtypeMap.hasOwnProperty(mxtype)) {
                            mxtypeEntries.push([mxtype, mxtypeMap[mxtype]]);
                        }
                    }
    
                    mxtypeEntries.sort(function(a, b) {
                        return b[1] - a[1];
                    });
    
                    var topMxtypes = mxtypeEntries.slice(0, 10);
                    for (var i = 0; i < topMxtypes.length; i++) {
                        result.barData.labels.push(topMxtypes[i][0] || "未知类型");
                        result.barData.quantities.push(topMxtypes[i][1]);
                    }
                    ...
                    for (var mxtype in mxtypeMap) {
                        if (mxtypeMap.hasOwnProperty(mxtype)) {
                            mxtypeEntries2.push([mxtype, mxtypeMap[mxtype]]);
                        }
                    }
    
                    mxtypeEntries2.sort(function(a, b) {
                        return b[1] - a[1];
                    });
    
                    var topMxtypes2 = mxtypeEntries2.slice(0, 8);
                    for (var i = 0; i < topMxtypes2.length; i++) {
                        result.pieData.labels.push(topMxtypes2[i][0] || "未知类型");
                        result.pieData.quantities.push(topMxtypes2[i][1]);
                    }
                    break;
        ...
        function updateDataTable() {
            ...
            if (!filteredData || filteredData.length === 0) {
                tbody.html('<tr><td colspan="11" class="text-center">暂无数据</td></tr>');  // colspan改为11
                updatePagination(0);
                return;
            }
            ...
            for (var i = startIndex; i < endIndex; i++) {
                ...
                var statusClass = item.Status === '已入库' ? 'badge-success' : 
                                  item.Status === '已出库' ? 'badge-warning' : 'badge-secondary';  // 修改状态判断
                ...
                row += '<td><span class="badge ' + statusClass + '">' + (item.Status || '') + '</span></td>';  // 修改状态显示
                row += '</tr>';
                tbody.append(row);
            }
        ...
        function exportToExcel() {
            ...
            var url = 'jinchutongji.aspx/ExportToExcel?mxtype=' + encodeURIComponent(mxtype || '') +
        ...
        function printReport() {
            ...
            setTimeout(function() {
                ...
                if (filteredData && filteredData.length > 0) {
                    ...
                    tableData += '<table>';
                    tableData += '<tr><th>日期</th><th>单号</th><th>商品名称</th><th>数量</th><th>金额</th><th>仓库</th><th>状态</th></tr>';

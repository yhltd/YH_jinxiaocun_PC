...
namespace Web.finance.model
{
    ...
    public class AccountingModel
    {
        ...
        public FinancePage<AccountingItem> getList(FinancePage<AccountingItem> financePage, string company, int classId)
        {
            ...
            string sql = "select (case len(code) when 4 then 'I' when 6 then 'II' when 8 then 'III' else '' end) as grade,*,isnull((SELECT SUM(money) FROM VoucherSummary WHERE VoucherSummary.code = a.code),0) as money,isnull((select name from Accounting as ac where ac.code = LEFT(a.code,4)),'') + isnull((select '-'+name from Accounting as ac where ac.code = LEFT(a.code,6) and ac.code != LEFT(a.code,4)),'') + isnull((select '-'+name from Accounting as ac where ac.code = LEFT(a.code,8) and ac.code != LEFT(a.code,6)),'') as fullName from (select *,ROW_NUMBER() over(order by LEN(code),id) as rownum from (select * from (SELECT *,LEFT(code, 1) AS class from Accounting) as t where t.class = @class) as c )as a where a.company = @company and a.rownum > @minPageParam and a.rownum < @maxPageParam";

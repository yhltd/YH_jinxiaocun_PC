...
namespace Web
{
    public partial class jinchutongji : System.Web.UI.Page
    {
        ...
        public static string GetAllSalesReturns(string mxtype = null, string startDate = null, string endDate = null)
        {
            try
            {
                ...
                MingxiModel model = new MingxiModel();
                var data = model.GetAllJinchuDetails(user.gongsi, mxtype, startDate, endDate);
                ...
                foreach (var item in data)
                {
                    ...
                    var formattedItem = new
                    {
                        ...
                        RawPrice = price,
                        RawAmount = amount,
                        Status = item.mxtype ?? ""  // 直接使用mxtype作为状态，不再转换
                    };
        ...
        [WebMethod]
        public static void ExportToExcel(string mxtype = null, string startDate = null, string endDate = null)
        {
            var user = HttpContext.Current.Session["user"] as yh_jinxiaocun_user;
            if (user == null) return;
            ...
            try
            {
                ...
                context.Response.AddHeader("Content-Disposition",
                    "attachment;filename=进出明细数据_" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + ".xls");
                ...
                sb.AppendLine("</head>");
                sb.AppendLine("<body>");
                sb.AppendLine("<h2>进出明细数据导出</h2>");
                sb.AppendLine("<p>导出时间：" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "</p>");

                if (!string.IsNullOrEmpty(mxtype) || !string.IsNullOrEmpty(startDate) || !string.IsNullOrEmpty(endDate))
                {
                    sb.AppendLine("<p>筛选条件：");
                    if (!string.IsNullOrEmpty(mxtype))
                        sb.Append("进出类型：" + mxtype + " ");
                ...
                foreach (var item in data)
                {
                    ...
                    if (item.shijian != null)
                        recordDate = item.shijian.Value.ToString("yyyy-MM-dd");

                    sb.AppendLine("<tr>");
                    sb.AppendLine("<td>" + index + "</td>");
                    sb.AppendLine("<td>" + recordDate + "</td>");
                    sb.AppendLine("<td>" + (item.mxtype ?? "") + "</td>");  // 直接显示进出类型
                ...
                sb.AppendLine("<td colspan='9' style='text-align: right;'>合计：</td>");
                sb.AppendLine("<td>" + totalAmount.ToString("F2") + "</td>");
                sb.AppendLine("<td colspan='2'></td>");
                sb.AppendLine("</tr>");

                sb.AppendLine("</table>");
                sb.AppendLine("<p>总计记录数：" + data.Count + " 条，总金额：" + totalAmount.ToString("F2") + "</p>");
                sb.AppendLine("</body>");
                sb.AppendLine("</html>");

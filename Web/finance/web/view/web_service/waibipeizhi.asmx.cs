...
namespace Web.finance.web.view.web_service
{
    ...
    public class waibipeizhi : System.Web.Services.WebService
    {
        private waibipeizhiService waibipeizhiService;

        [WebMethod]
        public string getList(string financePageJson, string huilv, string bizhong)  // 修改：参数名
        {
            try
            {
                waibipeizhiService = new waibipeizhiService();
                var financePage = FinanceJson.getFinanceJson().toObject<FinancePage<waibiPeizhi>>(financePageJson);
                financePage = waibipeizhiService.getList(financePage, huilv, bizhong);  // 修改：传递参数
        ...
        [WebMethod]
        public string getAllList(string huilv, string bizhong)  // 修改：参数名
        {
            try
            {
                waibipeizhiService = new waibipeizhiService();
                var list = waibipeizhiService.getAllList(huilv, bizhong);  // 修改：传递参数
        ...
        public string upd(string updInfo)
        {
            ...
            {
                waibipeizhiService = new waibipeizhiService();
                var entity = FinanceJson.getFinanceJson().toObject<waibiPeizhi>(updInfo);

                if (waibipeizhiService.upd(entity))
                {
        ...
        public string add(string simpleDataJson)
        {
            ...
            {
                waibipeizhiService = new waibipeizhiService();
                var entity = FinanceJson.getFinanceJson().toObject<waibiPeizhi>(simpleDataJson);

                if (waibipeizhiService.add(entity))
                {
